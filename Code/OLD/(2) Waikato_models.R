...
present$Cat500 <- sapply(1:nrow(present), function(x) ifelse(present$Category[x] < 500, "<500", ">=500")) #Categorise into two categories

#classifying absent data as both under500 and over500 so that <500 and >=500 have same impact of absences
absent1 <- absent %>% mutate(Cat500 = "<500") 
absent2 <- absent %>% mutate(Cat500 = ">=500")

#bind and group counts
obs_len500 <- rbind.data.frame(present, absent1, absent2) #bind all presences and absences
obs_len500 <- obs_len500 %>% select(Year, nzsegment, count, dist_i, long, lat, child_i, Cat500) %>% #select these variables
  group_by(Year, dist_i, long, lat, child_i, Cat500) %>% #group these together and then summarise by sum(count) in these groups
  summarise(count = sum(count))
obs_len500$CatNum <- sapply(1:nrow(obs_len500), function(x) ifelse(obs_len500$Cat500[x] == "<500", 1, 2)) #Number categories
table(obs_len500$Cat500, obs_len500$Year)
###
...
Data_len500 <- data.frame( "Catch_KG" = obs_len500$count, 
                           "Year" = as.numeric(obs_len500$Year),
                           "AreaSwept_km2" = (150/1000), 
                           "Lat" = obs_len500$lat, 
                           "Lon" = obs_len500$long, 
                           "Pass" = 0, 
                           "Knot" = obs_len500$child_i,
                           "Category" = obs_len500$Cat500,
                           "CategoryNum" = obs_len500$CatNum) %>% #include data in likelihood
...
pobs3 <- pnet +
  geom_point(data = Data_len500, aes(x = Lon, y = Lat, size = Density, color = Category), alpha = 0.6) +
  scale_color_brewer(palette = "Set1") +
  # guides(color = guide_legend(title = "Encounter")) +
  facet_wrap(.~Year)
ggsave(file.path(fig_dir, "Obs_length_cut500.png"), pobs3, height = 7, width = 8)




save.image(file.path(data_dir, "general_inputs_Waikato.Rdata"))
...
#Load data
load("./Waikato/Data/general_inputs_Waikato.Rdata")
...
#### Data input ######
Data_inp <- Data_len500 #the input data
...
#Saving directories
path <- file.path(getwd(), paste0("Waikato/Model1"))
...
# Log-predicted density by category (<500, >=500)
...
plot(as.vector(log(fit$Report$D_gct[,2,]))~ rep(c(2009:2017), each=nrow(network)),
     xlab="Year", ylab="Log(density)", main="Spread of log(density) of longfin eels >=500mm in the Waikato")
dev.off()
...
plot(as.vector((fit$Report$D_gct[,2,]))~ rep(c(2009:2017), each=nrow(network)),
     xlab="Year", ylab="Density", main="Spread of density of longfin eels >=500mm in the Waikato")
dev.off()
...
plot_maps_network(plot_set = 10,
                  ...
                  PlotName="Covariate_effects_poc_lessthan500",
...
plot_maps_network(plot_set = 10,
                  ...
                  category_names = c("<500mm",">=500mm"),
                  covar_names=covar_names_to_use,
                  PlotName="Covariate_effects_poc_greaterthan500",
                  PlotTitle = "Individual covariate effect P.O.C",
                  cex = 0.5,
                  pch=15,
                  n_p = n_p,
...
plot_maps_network(plot_set = 11,
                  ...
                  category_names = c("<500mm",">=500mm"),
                  covar_names=covar_names_to_use,
                  PlotName="Covariate_effects_pcr_lessthan500",
...
plot_maps_network(plot_set = 11,
                  ...
                  category_names = c("<500mm",">=500mm"),
                  covar_names=covar_names_to_use,
                  PlotName="Covariate_effects_pcr_greaterthan500",
                  PlotTitle = "Individual covariate effect P.C.Rs",
                  cex = 0.5,
                  pch=15,
                  n_p = n_p,
...
FieldConfig <-  c("Omega1" = 1, "Epsilon1" = 1, "Omega2" = 1, "Epsilon2" = 1)
RhoConfig <- c("Beta1" = 4, "Beta2" = 4, "Epsilon1" = 0, "Epsilon2" = 0) #spatio-temporal terms a random effect independent among years
...
#Saving directories
path <- file.path(getwd(), paste0("Waikato/Model2_allhab"))
dir.create(path, showWarnings = FALSE)
fig <- file.path(path, "Figures")
dir.create(fig, showWarnings = FALSE)
