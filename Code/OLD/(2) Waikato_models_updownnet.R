...
####
##Network
network <- readRDS(file.path(data_dir, "Waikato_updownnetwork.rds")) #%>% mutate('AreaSwept_km2'=dist_s*width)
...
obs <- readRDS(file.path(data_dir, "Waikato_observations_lf.rds")) #%>% 
#filter(Year!=2009) #habitat data not available for 2009
####
...
save.image(file.path(data_dir, "general_inputs_Waikato_updownnet.Rdata"))
...
#Load data
load("./Waikato/Data/general_inputs_Waikato_updownnet.Rdata")
...
#Saving directories
path <- file.path(getwd(), paste0("Waikato/Model1_updownnet"))
